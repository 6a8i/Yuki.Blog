...
public class PostRepository(IUnitOfWork uow) : IPostPorts
{
    ...
    public async Task<Post?> GetPostByIdAsync(Guid id, bool includeAuthor, CancellationToken cancellationToken)
    {
        string sql;
        ...
        else
        {
            ...
            var dbResults = await connection.QueryFirstOrDefaultAsync<PostEntity>(
                sql, 
                param: new
                {
                    Id = id
            ...
            if(dbResults is not null)
                return new(dbResults.Id, dbResults.Title, dbResults.Description, dbResults.Content, dbResults.AuthorId, default);
            else 
                return default;
        }
    }
